...
NEW_EXE = os.path.join(UPDATE_DIR, "Shrinkr-new.exe")


def _extract_new_exe():
    """Достаёт наш exe из скачанного zip в _update/Shrinkr-new.exe. Делаем это,
    пока приложение ещё работает: файл новый, блокировок нет. На любой глубине
    архива; иначе — первый .exe. Возвращает путь или None."""
    if not has_pending_update():
        return None
    try:
        want = os.path.basename(sys.executable).lower()   # напр. shrinkr.exe
        with zipfile.ZipFile(UPDATE_ZIP, "r") as zf:
            names = zf.namelist()
...
def apply_self_update(target):
    """Выполняется НОВЫМ exe (Shrinkr-new.exe), запущенным с --apply-update: ждёт,
...
def cleanup_applied():
    """При старте убирает Shrinkr-new.exe, если он уже совпадает с текущим exe
